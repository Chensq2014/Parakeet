...
public class NetWebModule : AbpModule
{
    ...
    {
        Log.Information($"{{0}}", $"{CacheKeys.LogCount++}、ConfigureBundles...ConfigureServices中的流程日志线程Id：【{Thread.CurrentThread.ManagedThreadId}】");
        //context.Services.Configure<BundlingOptions>()
        Configure<AbpBundlingOptions>(options =>
        {
            Log.Logger.Error($"{{0}}", $"{CacheKeys.LogCount++}、Configure配置{nameof(AbpBundlingOptions)} ScriptBundles:{BasicThemeBundles.Styles.Global},StyleBundles:{BasicThemeBundles.Scripts.Global}....ConfigureServices中的{options.GetType().Name}委托日志 线程Id：【{Thread.CurrentThread.ManagedThreadId}】");
            ...
            options.StyleBundles
                .Configure(LeptonXLiteThemeBundles.Styles.Global,
                bundle =>
                {
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        #endregion
        app.UseCors(CustomerConsts.AppName);
