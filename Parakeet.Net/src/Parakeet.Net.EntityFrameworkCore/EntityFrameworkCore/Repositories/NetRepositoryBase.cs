...
namespace Parakeet.Net.EntityFrameworkCore.Repositories
{
    /// <summary>
        ...
        where TEntity : EntityBase
    {
            ...
            List<string> propNames, Dictionary<string, SqlParameter[]> sqlParameterRows, StringBuilder sql, ref int parameterIndex)
        {
            ...
            var parameterSql = $@"insert into [{CustomerConsts.DefaultDbSchema}].[{CustomerConsts.DefaultDbTablePrefix}{tableName}s] ({string.Join(",", propNames.Select(name => $"[{name}]"))},[Id],[CreationTime]) values ({string.Join(",", rowParameters.Select(m => m.ParameterName))})";
        ...
        public async Task<bool> SqlUpdate<TEntity>(KeyValueDto<Guid, string> input)
        {
            ...
            var tableName = type.GetCustomAttribute<TableAttribute>()?.Name ?? type.Name;
            var sql = $"UPDATE [{CustomerConsts.DefaultDbSchema}].[{CustomerConsts.DefaultDbTablePrefix}{tableName}s] SET {setSqlParameter} WHERE Id=@Id;";
        ...
        public async Task<bool> SqlDelete<TEntity>(InputIdDto input)
        {
            var type = typeof(TEntity);
            var tableName = type.GetCustomAttribute<TableAttribute>()?.Name ?? type.Name;
            var sql = $"Delete From [{CustomerConsts.DefaultDbSchema}].[{CustomerConsts.DefaultDbTablePrefix}{tableName}s] WHERE Id=@Id;";
        ...
        public async Task<bool> BulkSqlDelete<TEntity>(InputIdsDto input)
        {
            ...
            {
                sql.Append($"Delete From [{CustomerConsts.DefaultDbSchema}].[{CustomerConsts.DefaultDbTablePrefix}{tableName}s] WHERE Id=@Id_{parameters.Count};");
        ...
        public async Task<TEntity> SqlQuery<TEntity>(InputIdDto input) where TEntity : BaseEntity, new()
        {
            ...
            var sql = $@"Select [Id],{columnsString} From [{CustomerConsts.DefaultDbSchema}].[{CustomerConsts.DefaultDbTablePrefix}{tableName}s] Where Id=@Id;";
        ...
        public async Task<IList<TEntity>> SqlQueryList<TEntity>(InputIdsDto input) where TEntity : BaseEntity, new()
        {
            ...
            var propInfos = await _cacheTypePropertyService.GetCachePropertyInfos(type);
            var columnsString = string.Join(",", propInfos.Select(p => $"[{p.GetColumnName()}]"));
            var sql = $@"exec(Select [Id],{columnsString} From [{CustomerConsts.DefaultDbSchema}].[{CustomerConsts.DefaultDbTablePrefix}{tableName}s] Where cast([Id] as varchar(36)) in (@Ids));";
        ...
        public async Task<IList<TEntity>> SqlQueryByCondition<TEntity>(Expression<Func<TEntity, bool>> func) where TEntity : BaseEntity, new()
        {
            ...
            string sql = $"SELECT [Id],{columnsString} FROM [{CustomerConsts.DefaultDbSchema}].[{CustomerConsts.DefaultDbTablePrefix}{type.GetMappingName()}s] WHERE {where}";
            return await ExecuteQuaryList(new KeyValuePair<string, SqlParameter[]>(sql, parameters.ToArray()), (command) =>
            {
                var entities = new List<TEntity>();
                var reader = command.ExecuteReader();
