...
namespace Parakeet.Net.EntityFrameworkCore
{
    ...
    public class SqlServerMigrationsDbContextFactory : IDesignTimeDbContextFactory<SqlServerMigrationsDbContext>
    {
        public SqlServerMigrationsDbContext CreateDbContext(string[] args)
        {
            ...
            var builder = new DbContextOptionsBuilder<SqlServerMigrationsDbContext>()
                .UseSqlServer(configuration.GetConnectionString(CustomerConsts.SqlServerConnectionStringName));
                //.UseLoggerFactory(new SerilogLoggerFactory());
                ////这会使所有查询都不被跟踪。 仍可添加 AsTracking() 来进行特定查询跟踪。
                //.UseQueryTrackingBehavior(QueryTrackingBehavior.NoTracking);

            return new SqlServerMigrationsDbContext(builder.Options);
        }
