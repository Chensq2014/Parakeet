...
using Parakeet.Net.PersonalCaches;
using System;
...
using Attachment = Parakeet.Net.ValueObjects.Attachment;
...
namespace Parakeet.Net.Needs
{
    ...
    public class NeedAppService : BaseNetAppService<Need>, INeedAppService
    {
        ...
        /// <param name="input"></param>
        /// <returns></returns>
        public async Task<Message> Create([FromForm]NeedCreateDto input)
        {
            try
            {
                // 验证码
                ...
                return new Message { Status = true, Url = $"url", Msg = "保存成功！" };
            }
            catch (Exception e)
            {
