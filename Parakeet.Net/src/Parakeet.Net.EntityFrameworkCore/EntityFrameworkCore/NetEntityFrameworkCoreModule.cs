...
using Parakeet.Net.EntityFrameworkCore.Repositories;
...
public class NetEntityFrameworkCoreModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        ...
        Log.Warning($"{{0}}", $"{CacheKeys.LogCount++}、Module启动顺序_{nameof(NetEntityFrameworkCoreModule)} End PreConfigureServices ....");
    ...
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        {
            Log.Error($"{{0}}", $"{CacheKeys.LogCount++}、AddAbpDbContex配置{nameof(IAbpDbContextRegistrationOptionsBuilder)}告诉容器 构造{nameof(NetDbContext)} 时把options当作参数传递给构造函数....ConfigureServices中的{options.GetType().Name}委托日志 线程Id：【{Thread.CurrentThread.ManagedThreadId}】");
